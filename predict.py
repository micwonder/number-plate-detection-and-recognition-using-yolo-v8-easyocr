...
def _main():
    ...
    if os.path.isdir(TARGET_PATH):
        ...
        for img_name in img_names:
            if not check_name(img_name): continue

            img_pathes.append(os.path.join(TARGET_PATH, img_name))

    else:
        img_pathes = [TARGET_PATH]
    ...
    for img_path in img_pathes:
        ...
        image = plot_bboxes(image, results[0].boxes.data, label_map, False, 0.7)

        image = cv2.resize(image,[1024, 768])
        cv2.imshow('license', image)
        cv2.waitKey(0)
